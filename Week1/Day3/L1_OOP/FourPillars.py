...
# Some methods can share the same name across child classes, but they operate differently based on that class. Thereby using the same naming of the methods to reduce complexity, but intended to act similarly.
    ...
    def Defend(self, dmg):
        ...
        self.hp -= dmg
        self.rage += 10
        print(f'{self.name} took damage and gained 10 rage! \n Rage : {self.rage} \n HP : {self.hp}')
        return self 
...
This is a way to simplify a process and limit what needs to be seen/known by a user to complete an action. (This extends to things like making a purchase on a website, the user doesn't need to know/see the database interactions or any of the code in order to make those purchases.)
'''
...
class Barista:
    def __init__(self,name):
        ...
        # Using Abstraction, the Barista would only need to know about what beans to use, and the CoffeeM would track the temperature that is used. Like an automated coffee brewer.
